document.addEventListener('DOMContentLoaded', () => {
    ...
    let action1 = null; // 追加: [保留アクション].001
    let action2 = null; // 追加: [保留アクション].002
    ...
    loader.load('nice.glb', function (gltf) {
        ...
        gltf.animations.forEach(function (clip) {
            ...
            if (clip.name === '[保留アクション]') {
                action1 = action; // 変更: [保留アクション] を action1 に設定
                action.loop = THREE.LoopRepeat; // 常に動かす
                action.clampWhenFinished = false;
                action.play(); // アニメーションを再生
            } else if (clip.name === '[保留アクション].001') { 
                currentAction = action; // 変更: [保留アクション].001 を currentAction に設定
                action.loop = THREE.LoopOnce;
                action.clampWhenFinished = true;
            } else if (clip.name === '[保留アクション].002') {
                action2 = action; // 変更: [保留アクション].002 を action2 に設定
